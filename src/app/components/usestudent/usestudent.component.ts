...
@Component({
  selector: 'app-usestudent',
  templateUrl: './usestudent.component.html',
  styleUrls: ['./usestudent.component.css']
})
export class UsestudentComponent implements OnInit {
  ...
  constructor(private reporteService:ReporteService,
    ...
    this.form = this.fb.group({
      ...
      nacionalidad: ['', [Validators.required, Validators.maxLength(200),Validators.pattern('^[A-Za-zñÑáéíóúÁÉÍÓÚ ]+$')]],
      nombreUsuario: ['', [Validators.required, Validators.maxLength(200), Validators.pattern('[t]+[j]+[.]+(?:[A-Za-zñÑáéíóúÁÉÍÓÚ]+)[.]+(?:[A-Za-zñÑáéíóúÁÉÍÓÚ]+)[.]+[A-Za-zñÑáéíóúÁÉÍÓÚ]+[@]+[u]+[p]+[d]+[s]+[.]+[n]+[e]+[t]+[.]+[b]+[o]')]],
  ...
  CambiarEstado(usuario: any, accion: number) {
    if(accion==0){
      Swal.fire({
        title: '¿Esta Seguro?',
        text: "Que quiere Eliminar a este Estudiante!",
        icon: 'warning',
          ...
          if (this.id != undefined) {
            ...
            this.usuarioservice
              ...
              .subscribe((r) => {
                Swal.fire(
                  'Eliminado!',
                  'El registro de Estudiante fue Eliminado Exitosamente.',
                  'success'
                )
                this.ListaUsuario();
              });
      ...
      Swal.fire({
        title: '¿Esta Seguro?',
        text: "Que quiere Activar a este Estudiante!",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
          ...
          if (this.id != undefined) {
            ...
            this.usuarioservice
              ...
              .subscribe((r) => {
                Swal.fire(
                  'Activado!',
                  'El registro de Estudiante fue Activado Exitosamente.',
                  'success'
                )
                this.ListaUsuario();
              });
  ...
  async listadoUsuario() {
    await this.usuarioservice.getUsuarioestudent().subscribe((data) => {
   ...
   reportePDF(){
    ...
    this.usuarioservice.getUsuarioestudent().subscribe(
      (data) => {
  ...
  //REPORTE EXCEL
 reporteExcel(){
  var nombreArchivo="reporteUsuariosEstudents.xlsx"
  var id = document.getElementById('datos');
  //console.log('datos');
  this.reporteService.reporteExcel(id,nombreArchivo,7);
 }
}
